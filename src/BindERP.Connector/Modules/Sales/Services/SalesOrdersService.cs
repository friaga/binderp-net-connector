...
internal class SalesOrdersService : BaseService, ISalesOrdersService
{
    public SalesOrdersService(IBindConnection connection) 
        : base(connection)
    {
    }
    ...
    public Task<Page<SalesOrderRecord>?> GetSalesOrdersPageAsync(SearchCriteria? criteria = null, CancellationToken cancellationToken = default)
    {
        return this.HttpClient.GetAsync<Page<SalesOrderRecord>>($"/api/Orders", criteria, cancellationToken);
    }
    ...
    /// <exception cref="HttpRequestException">Lanzada cuando ha ocurrido un error en el envio de la operación.</exception>
    /// <exception cref="TaskCanceledException">Lanzada cuando la operación es cancelada.</exception>
    public Task<SalesOrder?> GetSalesOrderAsync(Guid salesOrderID, CancellationToken cancellationToken = default)
    {
        return this.HttpClient.GetAsync<SalesOrder>($"/api/Orders/{salesOrderID}", cancellationToken);
    }
}
